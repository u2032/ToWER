 ...
 * @author Cédric Longo
 */
class PairingReportTest {

    @Inject
    private PairingReport.Factory _factory;
    ...
    void paramatersTest( ) throws Exception {
        ...
        final PairingReport report = _factory.create( new ObservableTournament( tournament ), round, false );
        // Exercice
    ...
    @Test
    @DisplayName( "Data source has expected match sorted by position" )
    void dataSourceTest( ) throws Exception {
        ...
        final ObservableRound round = new ObservableRound( tournament.getRounds( ).get( 0 ) );
        final PairingReport ladderReport = _factory.create( oTournament, round, false );
        // Exercice
        final JRMatchListDataSource dataSource = ladderReport.getJRDataSource( );
        // Verify
        assertThat( dataSource.getMatches( ).size( ) ).isEqualTo( 2 );
    ...
    @Test
    @DisplayName( "Data source has expected match sorted by name when requested" )
    void dataSourceByNameTest( ) throws Exception {
        // Setup
        final Tournament tournament = initTournament( );

        final ObservableTournament oTournament = new ObservableTournament( tournament );
        final ObservableRound round = new ObservableRound( tournament.getRounds( ).get( 0 ) );
        final PairingReport ladderReport = _factory.create( oTournament, round, true );
        // Exercice
        final JRMatchListDataSource dataSource = ladderReport.getJRDataSource( );
        // Verify
        assertThat( dataSource.getMatches( ).size( ) ).isEqualTo( 4 );
        assertThat( dataSource.getMatches( ).get( 0 ).getLeftTeamId( ) ).isEqualTo( 1 );
        assertThat( dataSource.getMatches( ).get( 1 ).getLeftTeamId( ) ).isEqualTo( 2 );
        assertThat( dataSource.getMatches( ).get( 2 ).getLeftTeamId( ) ).isEqualTo( 3 );
        assertThat( dataSource.getMatches( ).get( 3 ).getLeftTeamId( ) ).isEqualTo( 4 );
    }
